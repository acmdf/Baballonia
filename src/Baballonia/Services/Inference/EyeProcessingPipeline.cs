...
public class EyeProcessingPipeline(ILogger<EyeProcessingPipeline> logger, IEyePipelineEventBus eyePipelineEventBus) : DefaultProcessingPipeline, IDisposable
{
    ...
    unsafe float[]? RunInference(Mat collected)
    {
        ...
        if (res.is_error)
        {
            ...
            logger.LogError($"Inference error: {errorMsg}");
